...
export default function EkspedisiPage() {
    ...
    try {
      ...
      const response = await fetch("/api/activities", {
        ...
        body: JSON.stringify({
          ...
          isExpedition: formData.isExpedition,
        }),
      });
    ...
    try {
      ...
      const response = await fetch(`/api/activities/${selectedActivity.id}`, {
        ...
        body: JSON.stringify({
          ...
          imageUrl,
          date: combinedDateTime.toISOString(),
          isExpedition: editFormData.isExpedition,
        }),
      });
  ...
  const renderForm = (data, setData, imageFileSetter, errorMsg) => (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3 p-4 rounded-xl bg-black/20 border border-white/5 select-none mt-1">
        <input
          id="isExpedition"
          type="checkbox"
          checked={data.isExpedition}
          onChange={(e) => setData((p) => ({ ...p, isExpedition: e.target.checked }))}
          className="w-4 h-4 rounded border-white/20 text-[#ea580c] focus:ring-[#ea580c] bg-transparent cursor-pointer"
        />
        <label htmlFor="isExpedition" className="text-xs font-semibold text-zinc-300 cursor-pointer uppercase tracking-wider">
          Tandai Kegiatan ini sebagai Ekspedisi
        </label>
      </div>
    </div>
  );
